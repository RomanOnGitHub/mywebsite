...
## 2026-01-24: Build-time import slugs

### ⚠️ Ошибка расширения в Node импорте
В интеграциях build‑time нельзя импортировать несуществующий `slugs.js`, если исходник — `slugs.ts`.

**Правило:**
Для build‑time Node‑контекста используй `pathToFileURL()` и указывай фактический файл (`.ts`) или точку входа, которую реально резолвит Node.

**Суть:** Неверный путь ломает сборку целиком; проверяй, что файл реально существует.

---

## 2026-01-24: View Transitions cleanup

### ⚠️ Невалидное событие
`astro:before-swap` не существует — cleanup не вызывается, и обработчики/синглтоны протекают между навигациями.

**Правило:**
Используй только `astro:page-load` и `astro:after-swap` для пересоздания/очистки клиентского состояния.

**Суть:** Неверное событие = утечки + дублирование обработчиков.

---

## 2026-01-24: Самодостаточный TypeScript конфиг

### ⚡ Проблема с типами в IDE
При отсутствии `node_modules` или ограниченном доступе IDE ругается на `extends: "astro/tsconfigs/strict"` и `types: ["astro/client", "vite/client"]`.

**Правило:**
Делай `tsconfig.json` самодостаточным: без `extends` на пакеты, с явными `target/module/lib`, и обязательно `noEmit: true`, чтобы TypeScript не пытался писать файлы поверх исходников.

**Суть:** Самодостаточный `tsconfig` стабилизирует линтер и IDE даже в песочнице и при частичном окружении.
